...
class Pos extends MY_Controller
{
    ...
    function getSales($warehouse_id = NULL)
    {
        ...
        $now=date("Y-m-d");
        $yesterday=date('Y-m-d',(strtotime ( '-40 days' , strtotime ($now) ) ));
        ...
        if ($warehouse_id) {
            // die("&&&&");
            $this->datatables
                ->select($this->db->dbprefix('sales').".id as id, date, reference_no, ".$this->db->dbprefix('rooms').".name as room_name, "."biller, cashier, customer, grand_total, paid, (grand_total-paid) as balance, payment_status, '' as cemail")
                ->from('sales')
                ->join('rooms', 'rooms.id=sales.room_id', 'left')
                //  ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->where('DATE_FORMAT(sma_sales.date,"%Y-%m-%d") BETWEEN "' . $yesterday . '" and "' . $now . '"')
                ->group_by('sales.id');
...
//                ->group_by('sales.id');
    ...
    function index($sid = NULL)
    {
        ...
        $this->form_validation->set_rules('room', $this->lang->line("room"), 'trim|required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($customer->r_type=="nonresident"){
...
//            }
    ...
    function view($sale_id = NULL, $modal = NULL)
    {
        ...
        $this->data['message'] = $this->session->flashdata('message');
        $this->data['sale'] = $sale;
        $this->data['room'] = $this->rooms_model->getRoomByID($sale->room_id);
        $this->data['rows'] = $this->pos_model->getAllInvoiceItems($sale_id);
        $inv = $this->pos_model->getInvoiceByID($sale_id);
        $biller_id = $inv->biller_id;
        $customer_id = $inv->customer_id;
