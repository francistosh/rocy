...
class Reports extends MY_Controller
{
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ...
                ->where(array('products.iskitchen' => 0 , 'products.inventory' => 0));
    ...
    function inventory()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('Inventory Report')));
        $meta = array('page_title' => lang('products_report'), 'bc' => $bc);
        $this->page_construct('reports/inventory', $meta, $this->data);
    }
    
    function getInventoryProductsReport($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('products', TRUE);
        if ($this->input->get('product')) {
            $product = $this->input->get('product');
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ...
                ->join($pp, 'products.id = PCosts.product_id', 'left')
                ->order_by('products.name')
                ->where(array('products.iskitchen' => 0 , 'products.inventory' => 1));
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
          ...
          $this->datatables
                ->select("sales.date, sales.reference_no,rooms.name as biller, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) SEPARATOR '___') as iname, grand_total, paid, (grand_total-paid) as balance, IF(grand_total=paid,'paid','due') payment_status,pmethod as paid_by,CONCAT(sma_payments.mpesa_transaction_no,'',sma_payments.cost_center_no,'',sma_payments.cheque_no,'',sma_payments.cc_no) as transaction_no", FALSE)   //paid_by,transaction_id
        //         ->select("sales.date, sales.reference_no, biller, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) SEPARATOR '___') as iname, sum(sma_sale_items.subtotal) as grand_total, '0' as paid, (grand_total-paid) as balance, payment_status,pmethod as paid_by,CONCAT(mpesa_transaction_no,'',cost_center_no,'',cheque_no,'',cc_no) as transaction_no", FALSE)   //paid_by,transaction_id
            ...
            if ($department) {
				if($department=='bar'){
					...
					//$this->datatables->join('reception_payments', 'reception_payments.sale_id=sales.id', 'left')
...
$result=$this->db->query("SELECT * FROM (SELECT sma_sales.id,date,reference_no,total as credit,'0' as debit,'Invoice' as doc,IF(sma_sales.pos='1',GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '(', " . $this->db->dbprefix('sale_items') . ".quantity) SEPARATOR ')'),'Accomodation Invoice') as rmks FROM `sma_sales` LEFT JOIN sma_sale_items ON sma_sales.id = sma_sale_items.sale_id WHERE customer_id = '$user_id' AND pos='0' GROUP BY  sma_sales.id  UNION
SELECT sma_sales.id,date,reference_no,total as credit,'0' as debit,'Invoice' as doc,GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '(', " . $this->db->dbprefix('sale_items') . ".quantity) SEPARATOR ')') as rmks FROM `sma_sales` LEFT JOIN sma_sale_items ON sma_sales.id = sma_sale_items.sale_id WHERE customer_id = '$user_id' AND pos='1' GROUP BY  sma_sales.id UNION
SELECT sma_payments.id,sma_payments.date,sma_payments.reference_no,'0',sma_payments.amount,'Payment' as doc,paid_by FROM `sma_payments` JOIN sma_sales ON sma_sales.id = sma_payments.sale_id AND  sma_sales.customer_id = '$user_id' UNION 
SELECT sma_reception_payments.id,sma_reception_payments.date,sma_reception_payments.reference_no,'0',sma_reception_payments.amount,'Payment' as doc,paid_by FROM `sma_reception_payments` JOIN sma_sales ON sma_sales.id = sma_reception_payments.sale_id AND  sma_sales.customer_id = '$user_id' ) AS T1 ORDER BY date")->result();
...
//die();
     ...
     $company=$this->companies_model->getCompanyByID($user_id);
        ...
        $html='<table width="98%" border="1" style="margin-left:10px;border-collapse:collapse;font-size:14px">
            <tr><th width="20%"><img src="' . site_url() . 'assets/uploads/logos/logo.png" alt="' . SITE_NAME . '"  /></th><th style="text-align:center" colspan="5"><u>Statement of Account:<br><br>'.strtoupper($company->name).'</u></th></tr>
...
$html.='<tr style="cellpadding:2px">
  ...
  <td  style="text-align:right">'.number_format($runningbal).' &nbsp;</td></tr>';
$count++;
            }
            $html.="<tr><td colspan='3' style='text-align:center'><b>TOTALS</b></td><td style='text-align:right'><b>".number_format($totaldebit)."</td><td style='text-align:right'><b>".number_format($totalcredit)."</b></td><td style='text-align:right'><b>".number_format($totalcredit -$totaldebit)."</b></td></tr>
			<tr><td colspan='5' style='text-align:center'><b>AMOUNT DUE</b></td><td style='text-align:right'><b>".number_format($totalcredit -$totaldebit)."</b></td></tr>";
