...
class Billers extends MY_Controller
{
    ...
    function getBillers()
    {
        ...
        $this->datatables
            ...
            ->add_column("Actions", "<center><a class=\"tip\" title='" . $this->lang->line("edit_biller") . "' href='" . site_url('billers/edit/$1') . "'><i class=\"fa fa-edit\"></i></a> 
            <a href='#' class='tip po' title='<b>" . $this->lang->line("delete_biller") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('billers/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></center>", "id");
        //->unset_column('id');
    ...
    function add()
    {
        ...
        if ($this->form_validation->run() == true && $cid = $this->companies_model->addCompany($data)) {
            ...
            $biller_details_data = array(
                'biller_id'=> $cid,
                'acc_name'	=> $this->input->post('acc_name'),
                'bank_name'	=> $this->input->post('bank_name'),
                'branch'	=> $this->input->post('branch'),
                'dollar_acc_no'	=> $this->input->post('dollar_acc_no'),
                'ksh_acc_no'	=> $this->input->post('ksh_acc_no'),
                'swift_code'	=> $this->input->post('swift_code'),
                'mpesa_business_no'=> $this->input->post('mpesa_business_no'),
                'mpesa_acc_no'=> $this->input->post('mpesa_acc_no'),
            );
            if($this->companies_model->addBillerDetails($biller_details_data)){
    ...
    function edit($id = NULL)
    {
        ...
        $biller_details = $this->companies_model->getBillerByID($id);
        $this->form_validation->set_rules('name', 'Name', 'required');
        $this->form_validation->set_rules('company', 'Company', 'required');
        if ($this->form_validation->run('billers/edit') == true) {
        ...
        if ($this->form_validation->run() == true && $this->companies_model->updateCompany($id, $data)) {
            ...
            if($this->companies_model->updateBillerDetails($id,$biller_details_data)){
                $this->session->set_flashdata('message', $this->lang->line("biller_added"));
                redirect("billers");
            }else{
