...
class Quotes extends MY_Controller
{
    ...
    function add()
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$pax = $this->input->post('pax');
			 $fromdate = $this->sma->fld(trim($this->input->post('fromdate')));
			 $todate = $this->sma->fld(trim($this->input->post('todate')));
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $pr_item_discount = $this->sma->formatDecimal($pr_discount * $item_quantity * $item_days);
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        ...
                        $pr_item_tax = $this->sma->formatDecimal($item_tax * $item_quantity * $item_days);

                    }

                    $product_tax += $pr_item_tax;
                    $subtotal = (($item_net_price * $item_quantity * $item_days) + $pr_item_tax);
                    ...
                    $total += $item_net_price * $item_quantity * $item_days;
                }
            }
            if (empty($products)) {
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $biller_id = $this->input->post('biller');
            $status = $this->input->post('status');
			$att= $this->input->post('att_edit');
			$ref = $this->input->post('ref_edit');
			$pax = $this->input->post('pax_edit');
			$fromdate = $this->input->post('fromdate_dit');
