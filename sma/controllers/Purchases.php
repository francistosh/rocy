...
class Purchases extends MY_Controller
{
    ...
    function getPurchases($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("purchases.id as id,'' AS product_name,sma_purchases.date as date, reference_no, supplier,first_name, sma_purchases.status as status, grand_total, paid, (grand_total-paid) as balance, payment_status")
                ->from('purchases')
...
//generate pdf and force to download
    ...
    function getExpenses()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('expenses') . ".id as id, date, reference, amount, note, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as user,employee, attachment", FALSE)
            ->from('expenses')
