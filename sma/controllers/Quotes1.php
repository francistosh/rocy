...
class Quotes extends MY_Controller
{
    ...
    function getQuotes($warehouse_id = NULL)
    {
		...
		$reservation_link = anchor('quotes/reservation/$1', '<i class="fa fa-file-text-o"></i> ' . lang('Reservation_Voucher'));
        ...
        $convert_link = anchor('sales/add/$1', '<i class="fa fa-heart"></i> ' . lang('Create_Invoice'));
    ...
    function view($quote_id = NULL)
    {
        ...
        $this->data['rows'] = $this->quotes_model->getAllQuoteItems($quote_id);
		$this->data['pax'] = $this->quotes_model->getPaxfromquote($quote_id);
		//die($pax->qty);
	...
	function reservation($quote_id = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        $inv = $this->quotes_model->getQuoteByID($quote_id);
        //$this->sma->view_rights($inv->created_by);
        $this->data['rows'] = $this->quotes_model->getAllReservationItems($quote_id);
		$this->data['qcategory'] = $this->site->getCategoryByID($inv->category_id);
        $this->data['customer'] = $this->site->getCompanyByID($inv->customer_id);
        $this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('quotes'), 'page' => lang('quotes')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_quote_details'), 'bc' => $bc);
        $this->page_construct('quotes/reservation', $meta, $this->data);
    ...
    function reservationpdf($quote_id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        ...
        $this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
        $this->data['user'] = $this->site->getUser($inv->created_by);
        $this->data['title']="Reservation_Voucher";
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $name = $this->lang->line("reservation voucher") . "_" . str_replace('/', '_', $inv->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'quotes/reservationpdf', $this->data, TRUE);
        if ($view) {
            $this->load->view($this->theme . 'quotes/reservationpdf', $this->data);
        } elseif ($save_bufffer) {
            return $this->sma->generate_pdf($html, $name, $save_bufffer);
        } else {
            $this->sma->generate_pdf($html, $name);
	    ...
	    function transport()
    {
        ...
        $this->form_validation->set_rules('trcustomer', $this->lang->line("trcustomer"), 'required');
		$this->form_validation->set_rules('chkindate', $this->lang->line("chkindate"), 'required');
		$this->form_validation->set_rules('chkoutdate', $this->lang->line("chkoutdate"), 'required');
        ...
        if ($this->form_validation->run() == true) {
           ...
           // }
            ...
            $customer_id = $this->input->post('trcustomer');
            $biller_id = $this->input->post('biller');
			$categoryid =  $this->input->post('qcategory');
			$noofrooms = $this->input->post('nrooms');
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->addQuote($data, $products)) {
          ...
          //  $this->data['warehouses'] = $this->Owner ? $this->site->getAllWarehouses() : NULL;
            ...
            $this->page_construct('quotes/transport', $meta, $this->data);
        }
    }
    ...
    function add()
    {
		...
		$this->form_validation->set_rules('chkindate', $this->lang->line("chkindate"), 'required');
		$this->form_validation->set_rules('chkoutdate', $this->lang->line("chkoutdate"), 'required');
		$this->form_validation->set_rules('qcategory', $this->lang->line("qcategory"), 'required');
...
$this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true) {
			...
			$chkoutdate = $this->input->post('chkoutdate');
    ...
    function suggestions()
    {
		...
		$checkin_date =  $this->sma->fld(trim($this->input->get('checkin_date',TRUE)));
		$chkout_date =  $this->sma->fld(trim($this->input->get('chkout_date',TRUE)));
		$rtype = $this->input->get('rtype', TRUE);
		$datetime1 = new DateTime($checkin_date);

$datetime2 = new DateTime($chkout_date);
...
$daydifferebce = $difference->d;
        ...
        $rows = $this->quotes_model->getProductNames($sr, $warehouse_id,$checkin_date,$rtype);
        if ($rows) {
            foreach ($rows as $row) {
                $option = FALSE;
                $row->quantity = 0;
                ...
                $row->real_unit_price = $row->price * $daydifferebce;
				}
                $combo_items = FALSE;
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
