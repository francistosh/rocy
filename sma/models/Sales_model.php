...
class Sales_model extends CI_Model
{
    ...
    {
        $this->db->select('products.id, code, name, type, warehouses_products.quantity, price, nrsdnt_amount, tax_rate, tax_method')
    ...
    public function getInvoiceItemByNameAndInvoiceID($name,$id)
    {

        $q = $this->db->get_where('invoice_items', array('name' => $name,'sale_id'=>$id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getAllInvoiceItemsByRoomId($room_id)
    {
        $this->db->select('sale_items.id,sales.id as sale_id,sales.date,sales.reference_no,sale_items.product_name,sale_items.unit_price,sale_items.quantity,sale_items.subtotal,sales.currency')
            ->join('sales', 'sales.id=sale_items.sale_id', 'left')
            ->group_by('sale_items.id')
            ->order_by('id', 'asc');
        $q = $this->db->get_where('sale_items', array('sma_sales.room_id' => $room_id,'sma_sales.pos' => 1,'sma_sales.pmethod' => 'rooms','sma_sales.paid<sma_sales.grand_total'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	 ...
	 public function getUnpaidInvoiceItemsByRoomId($room_id)
    {
        $this->db->select('sale_items.id,sales.id as sale_id,sales.date,sales.reference_no,sale_items.product_name,sale_items.unit_price,sale_items.quantity,sale_items.subtotal,sales.currency')
            ->join('sales', 'sales.id=sale_items.sale_id', 'left')
            ->group_by('sale_items.id')
            ->order_by('id', 'asc');
        $q = $this->db->get_where('sale_items', array('sma_sales.room_id' => $room_id, 'sma_sales.date >' => '2021-10-01 00:00:00','sma_sales.pos' => 1,'sma_sales.pmethod' => 'rooms'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	 ...
	 public function getUnpaidBillsByRoomId($room_id)
    {
        $this->db->select($this->db->dbprefix('sales').".id as id,sales.id as sale_id, date,sales.id as reference_no, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as product_name, grand_total as unit_price, paid, (grand_total-paid) as subtotal, sales.currency,companies.name")
				->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
				->join('sma_roomstats', 'sma_roomstats.sale_id=sales.id', 'left')
				->join('sma_companies', 'sma_companies.id=sales.customer_id', 'left')
				//->where('sales.payment_status <> "paid"')
                ->group_by('sales.id');
		 $q = $this->db->get_where('sales', array('sma_sales.room_id' => $room_id, 'sma_sales.date >' => '2021-10-01 00:00:00','sma_sales.pos' => 1,'sma_sales.pmethod' => 'rooms','sma_sales.cleared' =>0,'sma_sales.grand_total > sma_sales.paid'));
	    ...
	    public function getAllInvoiceItemsByCustId($cust_id)
    {
        $this->db->select("sale_items.id,sales.id as sale_id,sales.date,sales.reference_no, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '(', " . $this->db->dbprefix('sale_items') . ".quantity) SEPARATOR ')') as product_name, grand_total as unit_price,sale_items.quantity,(sma_sales.grand_total - sma_sales.paid) as subtotal,sales.currency")   //paid_by,transaction_id
    ...
    public function getAllSalesByRoomId($room_id)
    {
        $this->db->select('sales.*')
            ->order_by('id', 'asc');
        $q = $this->db->get_where('sales', array('sma_sales.room_id' => $room_id,'sma_sales.pos' => 1,'sma_sales.payment_status' => 'due'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getAllInvoiceItemsByInvoiceId($sale_id) {
    ...
    public function addSale($data = array(), $items = array(), $payment = array())
    {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            /*this is where to postpone qty reduction*/
            $saledate=date("Y-m-d",  strtotime($data['date']));
            if( $saledate !=date("Y-m-d")){
             //postpone qty reduction
             $postponed_sale['sale_id']= $sale_id;
             $postponed_sale['date']= $saledate;
             $postponed_sale['completed']= 0;
            $this->db->insert('postponed_sale',$postponed_sale);
             }
                else{          
    ...
    public function addSaleInvoice($data = array())
    {
        if ($this->db->insert('sales', $data)) {
    ...
    public function addSaleInvoiceItem($data = array())
    {
    ...
    public function getSaleInvoiceItemsBySaleId($sale_id)
    {
        $this->db->select('*');
    ...
    public function updateSaleOnly($id, $data = array())
    {
        $this->db->where('id', $id);
    ...
    public function updateInvoiceItem($id, $data = array())
    {
        $this->db->where('id', $id);
        if ($this->db->update('invoice_items', $data)) {
            return true;
        }
        return false;
    }
    ...
    public function deleteSale($id)
    {
        ...
        $this->db->delete('invoice_items', array('sale_id' => $id)) &&
    ...
    public function deleteSaleLeavePayments($id,$date)
    {
        $sale_items = $this->resetSaleActions($id);
        if ($this->db->delete('invoice_items', array('sale_id' => $id)) && $this->db->update('sma_roomstats', array('check_out' => $date), array('sale_id' => $id)) ) {
    ...
    {
        $q = $this->db->get_where('sales', array('id' => $saleid), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getTOTALKES()
    {
        $this->db->select_sum('total');
        $q = $this->db->get_where('sales', array('currency' => "KES"));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    {
        $this->db->select_sum('total');
        $q = $this->db->get_where('sales', array('currency' => "US$"));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function addPayment($data = array())
    {
        ...
        if ($this->db->insert('payments', $data)) {
			  $q = $this->db->get_where('sales', array('id' => $data['sale_id']), 1);
            
        if ($q->num_rows() > 0) {
            $status="";
            //get total amount
              $amount=$q->row()->paid + $data['amount'];
             if($q->row()->grand_total <=$amount){
            $status="paid";
             }else if($q->row()->grand_total > $amount && $q->row()->paid == 0){
                $status="due";
             }else if($amount < $q->row()->grand_total   && $q->row()->paid >0 ){
                 $status="partial";
             }
			if($data['paid_by']=='nonchrg'){
						$q=$this->db->query('UPDATE sma_sales SET  bill_change= "0", payment_status="'.$status.'", total ="0",grand_total = "0",pmethod="'.$payment['paid_by'].'",
                              paid="0" WHERE id='.$data['sale_id']);
           
					} else if($data['paid_by']=='costcenter'){
					$q=$this->db->query('UPDATE sma_sales SET total='
                            .$amount.', bill_change="0", payment_status="'.$status.'",pmethod="'.$data['paid_by'].'",
                              paid="0" WHERE id='.$data['sale_id']);	
					}
					else {
                    $q=$this->db->query('UPDATE sma_sales SET bill_change="0", payment_status="'.$status.'",pmethod="'.$data['paid_by'].'",
                              paid='.$amount.' WHERE id='.$data['sale_id']);
					}
		}
            if ($this->site->getReference('pay') == $data['reference_no']) {
                $this->site->updateReference('pay');
 ...
 public function getPaxfrominvoice($id)
    {
        $this->db->select('sum(quantity) as qty');
               //$this->db->order_by('id', 'asc');
        $q = $this->db->get_where('sale_items', array('sale_id' => $id,'product_code <>' => "%TLC%"));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
