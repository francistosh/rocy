...
class Companies_model extends CI_Model
{
    ...
    public function getBillerByID($id)
    {
        ...
        biller_details.biller_id,
        biller_details.acc_name,
        biller_details.bank_name,
        biller_details.branch,
        biller_details.dollar_acc_no,
        biller_details.ksh_acc_no,
        biller_details.swift_code,
        biller_details.mpesa_business_no,
        biller_details.mpesa_acc_no');
        $this->db->from('companies');
        $this->db->join('biller_details', 'companies.id = biller_details.biller_id');
    ...
    public function addCompany($data = array())
    {
        ...
        return false;
		 ...
		 public function addHouseKeeping($data = array())
    {
        if ($this->db->insert('house_keeping', $data)) {
            $cid = $this->db->insert_id();
            return $cid;
        }
        return false;
    ...
    public function addBillerDetails($data = array())
    {
        if ($this->db->insert('biller_details', $data)) {
    ...
    public function updateBillerDetails($id, $data = array())
    {
        $this->db->where('biller_id', $id);
        if ($this->db->update('biller_details', $data)) {
            return true;
        }
        return false;
    }
    ...
    public function deleteBiller($id)
    {
        ...
        if ($this->db->delete('biller_details', array('biller_id' => $id)) && $this->db->delete('companies', array('id' => $id, 'group_name' => 'biller'))) {
            return true;
        }
        return FALSE;
    }
    ...
    public function getCustomerSuggestions($term, $limit = 10)
    {
        ...
        $this->db->where('customer_group_name' , 'individual');
