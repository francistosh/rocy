...
SELECT `sma_products`.`portion7`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion7qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion7`) T6 GROUP BY `id`)T8 WHERE id = '$pro'  ");
    ...
    public function getCompanyByGroupID($customer_group_id) {
        $q = $this->db->get_where('companies', array('customer_group_id' => $customer_group_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getAllRooms($roomtype) {
		
		$this->db->select('rooms.id as id, rooms.category_type , rooms.name , categories.name as catname,roomstats.status')
            ->join('categories', 'categories.id=rooms.category_type', 'left')
			->join('roomstats', 'roomstats.room_id=rooms.id', 'left');
			
          		if($roomtype) {
            $this->db->where('categories.code', $roomtype);
        }
        $q = $this->db->get('rooms');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
		...
		$this->db->select('rooms.id as id, rooms.category_type , rooms.name , categories.name as catname')
            ->join('categories', 'categories.id=rooms.category_type', 'left');
	...
	public function getVouchermaxcode() {
		$this->db->select('max(id)+1 as id ');
        $q = $this->db->get('sma_vouchers');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    }
	    public function getRoombydate($date) {
    ...
    public function getAllInvCategories() {
		//get rooms category
        $this->db->order_by('name');
        $q = $this->db->get_where('categories', array('inv' => '1'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getCategoryByID($id) {
        $q = $this->db2->get_where('categories', array('id' => $id), 1);
        if(is_object($q)){
            if ($q->num_rows() > 0) {
                return $q->row();
            }
	    ...
	    public function getCategoryBycode($id) {
        $q = $this->db2->get_where('categories', array('code' => $id), 1);
        if(is_object($q)){
            if ($q->num_rows() > 0) {
                return $q->row();
            }
    ...
    public function getUpcomingEvents() {
        $dt = date('Y-m-d');
        $this->db->where('check_in >=', $dt)->order_by('check_in')->limit(5);
    ...
    public function syncProductQty($product_id, $warehouse_id) {
        ...
        $this->db->update('products', array('quantity' => $balance_qty), array('id' => $product_id),  array('iskitchen' => 0));
