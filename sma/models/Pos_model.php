   ...
   public function poslogin($userid, $password){
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL)
    {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            $this->postSaleInvoice($data);  //post to erp
...
//$this->db->dbprefix('tablename');
        ...
        $saleinvoice["account"]=100101; //SALES GL CODE
        ...
        $saleinvoice["type_no"]=1;
         $saleinvoice["tran_date"]=date("Y-m-d");
        $saleinvoice["account"]=1200;   //AR GL CODE
     ...
     unset($saleinvoice);
        ...
        $saleinvoice["type_no"]=1;
         $saleinvoice["tran_date"]=date("Y-m-d");
        $saleinvoice["account"]=2150; //SALES VAT ACCOUNT GL
     ...
     unset($saleinvoice);
       ...
       //payment DR Cash
        ...
        $saleinvoice["type_no"]=1;
         $saleinvoice["tran_date"]=date("Y-m-d");
        $saleinvoice["account"]=1065;  //DEBIT CASH 
    ...
    public function getRegisterCCSales($date,$enddate, $user_id = NULL)
    {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('payments.type', 'received')->where('sma_payments.date  between "'.$date.'" AND "'.$enddate.'"')->where('paid_by', 'CC');
        $this->db->where('payments.created_by', $user_id);
        ...
        if ($q->num_rows() > 0) {
            ...
            $cleared=1;
                ...
                $cleared=0;
                    ...
                    $q=$this->db->query('UPDATE sma_sales SET paid = '.$amount.', cleared = '.$cleared.'  WHERE id='.$payment['sale_id']);
