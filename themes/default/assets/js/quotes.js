...
$(document).on('click', '.qudel', function () {
     ...
     * Show manual item addition modal 
     ...
     $(document).on('click', '#addItemManually', function (e) {
        ...
        quitems[mid] = {"id": mid, "item_id": mid, "label": mname + ' (' + mcode + ')', "row": {"id": mid, "code": mcode, "name": mname, "quantity": mqty, "days":mdays, "price": unit_price, "unit_price": unit_price, "real_unit_price": unit_price, "tax_rate": mtax, "tax_method": 0, "qty": mqty, "type": "manual", "discount": mdiscount, "serial": "", "option":""}, "tax_rate": mtax_rate, "options":false};
        localStorage.setItem('quitems', JSON.stringify(quitems));
        loadItems();
        $('#mModal').modal('hide');
        $('#mcode').val('');
...
/* --------------------------
     * Edit Row No of rooms Method 
     -------------------------- */
	roomcount = 1;
	  $(document).on("focus", '.romquantity', function () {
       // room_old = $(this).val();
    }).on("change", '.romquantity', function () {
         var sum = 0;
    $(".romquantity").each(function(){
        sum += +$(this).val();
    });
	roomcount = sum;
...
function loadItems() {
    ...
    if (localStorage.getItem('quitems')) {
        ...
        $.each(quitems, function () {
            ...
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity,item_days = item.row.days, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            ...
            product_discount += parseFloat(item_discount * item_qty*item_days);
            ...
            if (site.settings.tax1 == 1) {
                if (pr_tax !== false) {
                    ...
                    product_tax += pr_tax_val * item_qty*item_days;
                }
            }
			...
			tr_html += '<td><input class="form-control text-center romquantity" name="rooms[]" type="text" value="' + formatDecimal(item_days) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="rooms_' + row_no + '" onClick="this.select();" ></td>';
            ...
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty) * parseFloat(item_days))) + '</span></td>';
           ...
           //$("#quTable tbody").sortable();
            total += formatDecimal(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)* parseFloat(item_days)));
